...
output_txt.append("Loading dataset...")
...
best_eps = best_min_samples = best_labels = None

# Iterate over parameter combinations
output_txt.append(f"Starting DBSCAN clustering at {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}")
for eps in eps_values:
    for min_samples in min_samples_values:
        ...
        if n_clusters > 1:
            ...
            output_txt.append(
                f"Clusters: {n_clusters}, eps: {eps}, min_samples: {min_samples}, Silhouette: {silhouette_avg}, DB Score: {db_score}, Combined Score: {combined_score}")
...
if best_labels is not None and len(np.unique(best_labels)) > 1:
    ...
    plt.colorbar(scatter, ax=ax, pad=0.1, label='Cluster Label')
    ax.set_title(f'DBSCAN Clustering with eps={best_eps}, min_samples={best_min_samples}')
