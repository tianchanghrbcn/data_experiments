...
public class BKTree<T>{
     ...
     *         满足距离范围的所有元素
     */
    public ArrayList<String[]> query(T term, double radius) {

        //Set<T> results = new HashSet<T>();
        ArrayList<String[]> list = new ArrayList<String[]>();
        if (root != null) {
            list = root.query(metricSpace, term, radius, new HashSet<T>(), list);
        }
    ...
    private static final class Node<T> {
        ...
        public ArrayList<String[]> query(MetricSpace<T> ms, T term, double radius,
                                         Set<T> results, ArrayList<String[]> list) {
            ...
            if (distance <= radius) {
                ...
                s[0] = Double.toString(distance);
                s[1] = (String)this.value;
            ...
            for (double i = Math.max(distance - radius, 1); i <= distance + radius; ++i) {
                ...
                // 递归调用
                if (child != null) {
                    child.query(ms, term, radius, results, list);
                }
